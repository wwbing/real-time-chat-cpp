...
{
    //tcp成功连接信号槽
    //使用lamda表达式处理，信号的接收方是lamda表达式本身，而非习惯性的this
    QObject::connect(&_socket, &QTcpSocket::connected, [&]() {
           qDebug() << "Connected to server!";
           // 连接建立成功后发送消息，logindialog接收
    ...
    QObject::connect(&_socket, &QTcpSocket::readyRead, [&]() {
            ...
            //遍历解析，因为qt封装的tcp的读取是非阻塞的，只要有数据就会发出readyRead信号，所以需要自己做一下处理判断
            forever {
            //先解析头部
            if(!_b_recv_pending){
...
void TcpMgr::slot_tcp_connect(ServerInfo si)
{
    ...
    //连接，连接成功、失败、断开连接，都会自动发出信号。QTcpSocket::connected，&QTcpSocket::disconnected，等
    _socket.connectToHost(si.Host, _port);
}

//处理tcp连接（chatserver）发送数据的信号槽
