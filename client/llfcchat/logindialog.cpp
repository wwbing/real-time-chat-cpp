...
void LoginDialog::initHttpHandlers()
{
    ...
    _handlers.insert(ReqId::ID_LOGIN_USER, [this](QJsonObject jsonObj){
        //参数jsonObj是GateServer从StatusServer查找到的ChatServer的连接地址
...
//获得GateServer中httpconnection携带回来的ChatServer连接信息的http回应
void LoginDialog::slot_login_mod_finish(ReqId id, QString res, ErrorCodes err)
{
    ...
    //调用对应的逻辑,根据id回调。
    //通过http协议登录成功后，需要根据tcp协议建立聊天的连接
    _handlers[id](jsonDoc.object());

    return;
}

//连接成功后sig_con_success信号的槽函数，用于连接成功后的登录（发送登录信息）
