...
//返回的是成员变量，使用的引用返回，这里也必须引用返回，因为socket必须是这个，值传递的复制返回就不行了
...
//读取body，读完了先将消息投递到逻辑队列中，再继续监听读取头
...
void CSession::asyncReadLen(std::size_t read_len, std::size_t total_len, std::function<void(const boost::system::error_code&, std::size_t)> handler)
{
	...
	_socket.async_read_some(
		...
		[read_len, total_len, handler, self](const boost::system::error_code& ec, std::size_t  bytesTransfered)->void {
			if (ec) {
				// 出现错误，调用回调函数
