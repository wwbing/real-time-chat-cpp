...
//CSession把收到的消息放到消息队列中，放进队列后条件变量会通知_worker_thread线程来处理
...
//下面5个函数是5个回调的具体实现，都满足_fun_callbacks中FunCallBack的函数签名
//登录验证回调函数
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short &msg_id, const string &msg_data) {
	...
	//析构的时候才会执行传入的函数，所以后面对rtvalue的赋值操作会成功被这个函数捕获
